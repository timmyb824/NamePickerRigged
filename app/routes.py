"""Route handlers, auth helpers, and the spin/rig logic.
...
def _templates(request: Request):
    """Return the Jinja templates instance stored on app state."""
...
async def create_wheel(
    ...
    if len(parsed := _parse_names(names)) < MIN_NAMES:
...
@router.post("/w/{code}/api/spin")
async def spin(code: str) -> JSONResponse:
    """Compute a spin result.

    Rigged spins are indistinguishable from random ones in the response.
    """
    wheel = _get_wheel_or_404(code)
...
@router.get("/w/{code}/admin", response_class=HTMLResponse)
...
@router.post("/w/{code}/admin/login")
...
@router.post("/w/{code}/admin/logout")
...
@router.post("/w/{code}/admin/content")
...
@router.post("/w/{code}/admin/delete")
